...
AGENTS = {
    "01_mobile_ux": {"label": "モバイル体験", "color": "#FF6B6B", "icon": "📱"},
    "02_visual_taipa_ux": {"label": "ビジュアル&タイパ", "color": "#4ECDC4", "icon": "👁️"},
    "03_onboarding_ux": {"label": "オンボーディング", "color": "#A78BFA", "icon": "🚪"},
    "04_pm": {"label": "PM", "color": "#F59E0B", "icon": "👔"},
    "05_engineer": {"label": "エンジニア", "color": "#34D399", "icon": "🛠"},
    "06_pm_assistant": {"label": "PM補佐", "color": "#F472B6", "icon": "🔍"},
}

UX_AGENTS = ["01_mobile_ux", "02_visual_taipa_ux", "03_onboarding_ux"]
...
PMとして、このお題でUI設計を始める前に、情報が不足していないか確認してください。
...
不足がなければ「情報は十分です。会議を始めます。」と言ってください。
不足があれば、ユーザーに聞きたい質問を**最大3つ**、箇条書きで簡潔に聞いてください。
「〜について教えてください」の形式で。""")
    ...
    add_message("04_pm",
        f"会議を始めます。\n\n"
        f"**お題：{topic}**\n\n"
        f"まず各自、専門領域からUI案を出してください。\n"
        f"**アプリ名・コア体験・画面構成・第一印象の仕掛け**の4点に絞って簡潔に。",
        "facilitator")
    time.sleep(1)

    run_parallel_ux(lambda aid: f"""お題: {topic}

PMの指示：「アプリ名・コア体験・画面構成・第一印象の仕掛けの4点に絞って簡潔に。」

あなたの専門視点から1案を提案してください。各項目3行以内で。""")
...
PM補佐として、PMが方針を決める前に情報補足してください：
- 提案に関連する事実・データ・類似アプリの事例があれば提供
- ターゲット（高校生）の実態とズレている点があれば指摘
- 3案を横断して見た時の盲点があれば指摘
...
PM補佐として軌道チェックしてください：
- PMの方針はゴール（高校生が見た瞬間やりたいと思い、継続的に使いたくなるUI）に合っているか
- 機能を盛りすぎていないか、シンプルさを保てているか
- 問題なければ「✅ 方針チェックOK」、問題あれば「⚠️ 軌道チェック」のフォーマットで
...
PM補佐として：
- レビュー内容がゴール（高校生の第一印象＋継続利用）に沿っているか
- 見落とされている重要な問題はないか
